...
function sortArr( comparator, array ){
    ...
    for(var i = (array.length - 1); i >= 0; i--) {
        for(var j = 1; j <= i; j++) {
            if(comparator(newArr[j-1], newArr[j])) {
                var temp = newArr[j];
                newArr[j] = newArr[j-1];
                newArr[j-1] = temp;
            }
...
function typeComparator( auto1, auto2){
    /* your code here*/
    var typeKey = ["Wagon", "SUV", "Pickup", "Roadster"];

    car1 = typeKey.indexOf(auto1.type);
    car2 = typeKey.indexOf(auto2.type);
...
typeSort.forEach(function(Automobile) {
